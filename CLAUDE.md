...
### フェーズ δ-1: スコアシステム見直し (完了, 2026-06-24)

Workflow で 5 案 (A: 正規化 0〜100% / B: グレード制 / C: ハイブリッド / D: 難易度補正 / E: 保守的最小差分) を 5 観点 (公平性 / ネット対戦適合 / 練習 UX / 単純さ / 学習適性) × 3 票で独立判定 → **案 A (正規化スコア)** が平均 8.67 で勝者 (E 7.43、C 7.17、B 6.27、D 6.03)。C/E から要素を取り込み (PERFECT 演出のフラグ分離、Score フィールド温存 + ネット交換契約維持) 統合案を実装。

採用判断 (ユーザー選択):
- PERFECT 演出範囲: **Game1/2/3 すべて** (練習モードもリアルタイム表示、学習用即時フィードバック優先)
- PERFECT 判定しきい値: 当初 `PERFECT_FRAME_TOLERANCE 2.0f` (緩め) で実装したが、3 秒目標で誤差率 1.1% (スコア 99.7) でも PERFECT 表示が出てしまい違和感あり。同日中に **`PERFECT_SCORE_THRESHOLD 99.95f` (スコア表示 "100.0" 完全整合)** に差し替え。画面に "100.0" と見える条件 = PERFECT で違和感ゼロ、ただし倍速 4.8 や長目標では事実上「フレーム単位ピッタリ」のみ到達可能で希少価値を担保
- 倍速 (CalMulti) 表示タイミング: **従来通り DONE 後のみ** (Game1/2 のゲーム性を維持、問題点 (5) は DONE 時併記で視覚補完)
- Game2 ネット対戦の floorf 整数化: **除去** (float 直接比較、0〜100 スケールで 95.7 vs 95.3 を別判定)

実装内容:
- [GameSceneMain.h](Project2/GameSceneMain.h): `TIMER_STATE` から `ScMulti` 削除、`BOOL IsPerfect` 追加。`Score` の意味を「正規化達成率 (0〜100、目標時間によらず満点 100 固定)」に再定義 (フィールド名・型・ネット交換 `%f` 1 個契約は維持)
- [GameSceneMain.cpp](Project2/GameSceneMain.cpp): `PERFECT_BONUS_RATIO 1.25f` 削除、`SCORE_MAX_RATE 100.0f` + `PERFECT_FRAME_TOLERANCE 2.0f` 追加。`#include <math.h>` 追加 (fabsf 用)。`timerReset` に `IsPerfect = FALSE` 初期化追加。`timerCalcScore` 本体を 3 分岐 if-else から `Score = max(0, 1 - |誤差|/CalFrame) × 100` の正規化式 + 連続関数 + `IsPerfect` フラグ算出に置換 (ピッタリの不連続段差を描画演出に分離)
- [Game1Scene.cpp](Project2/Game1Scene.cpp): `renderGame1Scene` の DONE 時に `state.IsPerfect == TRUE` の場合 "PERFECT!" (ColorRed) を `LAYOUT_X_DEFAULT + 200, LAYOUT_Y_SCORE` に追加描画
- [Game2Scene.cpp](Project2/Game2Scene.cpp): 同様に DONE 時 PERFECT 演出追加。`netStatus == NET_STATUS_RECEIVED` 時の勝敗比較から `floorf(scJudge)` / `floorf(state.Score)` を除去し float 直接比較化 (引き分けは事実上ピッタリ同点のみ)。`netBattle` 内コードは不変 (送信値の意味だけ更新)
- [Game3Scene.cpp](Project2/Game3Scene.cpp): MEASURING 中の `tmp` コピー方式維持、`tmp.IsPerfect == TRUE` でリアルタイム "PERFECT!" 表示。ScMulti 削除に伴うコメント文言更新 (`state.ScMulti/Score` → `state.Score/IsPerfect`)

解消した問題点 (δ-1 で明文化された 7 項目のうち 6 項目):
- (1) 目標時間バイアス → 正規化で構造的に解消 (1 秒目標も 19 秒目標も満点 100)
- (2) 短い目標時間プレイヤーの構造的不利 → 同上
- (3) ピッタリ時の不連続ジャンプ → スコア式は連続関数、不連続は IsPerfect 演出に分離
- (4) ネット対戦の不公平 → 双方 0〜100 同一スケールで直接比較可能
- (6) 練習モードのリアルタイムスコア値域変動 → 0〜100 固定で UX 安定
- (7) 大外し時の負スコア → `if (rate < 0) rate = 0` の床導入で防止
- (5) 倍速 (CalMulti) スコア無反映 → **未解消** (公平性最優先のため意図的に非反映、render 側の倍速併記で視覚補完にとどめる)

ネット交換プロトコルへの影響: γ-3 の `snprintf("%f", state.Score)` 1 個固定は維持、送信値の意味のみ「達成率 0〜100」に更新。δ-3 で `SCORE:%.2f` プレフィックス化する際の `%.2f` 精度が値域縮小により適切化。後方互換性: 旧 Score (0〜1425) 送信版との対戦は無意味になる (新版が常に低スコア扱い)。
